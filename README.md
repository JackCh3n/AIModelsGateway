...
- **管理后台登录保护（可选）**：服务器默认监听 0.0.0.0，在「设置」页配置管理员密码后，访问管理后台 API 需登录，防止局域网内他人读取上游 Key / 修改配置
- **暗色主题**：支持白天/夜间主题切换
...
服务器默认监听 `0.0.0.0`（局域网可访问），而管理后台 API 会返回全部上游 Key 等敏感信息。建议在「设置」页填写「管理员密码」并保存：

- 设置密码后，访问管理后台会自动弹出登录框，登录会话有效期 12 小时
- 密码以 SHA-256 摘要存储，不会明文落盘或回传给前端
- 需要取消保护时，在设置页点击「关闭登录保护」并保存即可
